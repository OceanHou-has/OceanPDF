...
- **后端解析服务** (pdf_parser.py)
  ...
  - 按页面组织数据结构
  - 保存为JSON格式：`storage/parsed/{pdf_name}/{pdf_name}_parsed.json`

- **DPS版面分析 + OCR**（已完整实现）
  - 调用外部DPS服务获取版面框与OCR聚合文本
  - 保存为JSON格式：`storage/parsed/{pdf_name}/{pdf_name}_dps.json`
...
- **标注页面** (PDFAnnotation.vue)
  ...
  - **DPS预览**
    - DPS结果在前端以预览方式展示（框 + 标签 + tooltip），不参与后端标注写入
...
- **论文版面分析器** (paper_analyzer.py)
  ...
  - **坐标系统**: 
    - DPS使用自己的坐标系（基于渲染图片尺寸，通常为1190x1587等）
    - PDF使用原始坐标系（如595.276x793.701）
    - 分析器会自动从DPS页面数据的 `width` 和 `height` 字段获取DPS坐标系尺寸
    - 然后加载parsed.json获取PDF原始尺寸，计算缩放比例进行坐标转换
  - **两次聚类算法**:
  ...
  - **坐标转换**: 强段落宽度和栏X坐标都会从DPS坐标系转换为PDF坐标系
  - **元数据存储**: 分析结果保存到 `document_metadata.json`（已转换为PDF坐标系）
...
- **DPS模式阅读顺序查看**
  - DPS模式下可以查看版面分析生成的阅读顺序
  - 显示阅读顺序徽章，但不可修改（只读模式）
  - 不显示手动排序按钮
  - 点击徽章显示提示："DPS阅读顺序: X（仅查看，不可修改）"
...
- **翻译配置对话框** (dialogs/TranslationConfigDialog.vue)
  ...
  - **翻译执行**
    ...
    - 翻译结果逐条写入 `storage/parsed/{pdf_name}/{pdf_name}_translation*.json`
    - **聚合块翻译拆分增强**：聚合文本内插入 `<<<OCEANPDF_SPLIT>>>` 分隔标记并强制保留，用于稳定拆分回填
...
- **预翻译文件生成**
  ...
  - 生成位置：`storage/parsed/{pdf_name}/{pdf_name}_pretranslation.json` 或 `_pretranslation_dps.json`
...
✅ `POST /api/v1/annotation/batch` - 批量标注PDF元素（性能优化，支持合并标注）
...
✅ `POST /api/v1/sorting/auto` - 自动计算阅读顺序
✅ `POST /api/v1/sorting/save` - 保存手动排序结果
...
- ✅ 翻译结果逐条落盘（translation.json实时更新）
- ✅ 翻译执行界面（双栏对照显示）
- ✅ 翻译控制功能（暂停/继续/停止）
- ✅ 加载已有翻译结果
- ✅ 队列模式翻译（支持真正的暂停/停止）
- ✅ 聚合块分隔标记 `<<<OCEANPDF_SPLIT>>>` + 译文分配回填稳定化

**测试脚本**：
- `backend/test_translation.py` 支持SSE模式打印实时进度（USE_SSE_TRANSLATE=true）
...
### 2. PDF解析模块
**当前已实现**:
- **PyMuPDF行级文本提取**: bbox+文本+字体信息，保存 `{pdf_name}_parsed.json`
...
- **数据存储**: 分别保存 `{pdf_name}_parsed.json` 和 `{pdf_name}_dps.json`
...
✅ POST   /api/v1/translation/translate/async            # 创建异步翻译任务（返回task_id）
✅ GET    /api/v1/translation/progress/{task_id}         # SSE推送翻译进度与单条结果
✅ GET    /api/v1/translation/result/{pdf_name}          # 获取翻译结果文件内容
✅ POST   /api/v1/translation/control/{task_id}/pause    # 暂停翻译任务
✅ POST   /api/v1/translation/control/{task_id}/resume   # 继续翻译任务
...
4. **PyMuPDF解析** → 行级文本提取
5. **保存JSON** → storage/parsed/{pdf_name}/{pdf_name}_parsed.json
6. **调用DPS服务** → 获取版面框（可选OCR：由 `with_ocr` 控制）
7. **DPS标记阅读顺序** → 每个box添加 `DPS_block_id` 和 `reading_order` 字段
8. **保存DPS JSON** → storage/parsed/{pdf_name}/{pdf_name}_dps.json
...
13. **批量标注** → 支持单选/框选，批量保存优化性能（仅Python结果可写回后端）
